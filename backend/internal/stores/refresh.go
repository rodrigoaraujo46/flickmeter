...
func (s refreshStore) Create(refresh refresh.Refresh, c context.Context) error {
	...
	_, err := s.db.Exec(ctx, query, refresh.UUID, refresh.User.Email)
...
func (s refreshStore) Read(uuid string, c context.Context) (refresh.Refresh, error) {
	...
	ref.UUID = uuid
	if err := s.db.QueryRow(ctx, query, uuid).Scan(&ref.User.Email, &ref.User.Username, &ref.User.AvatarURL); err != nil {
		if err == pgx.ErrNoRows {
			return refresh.Refresh{}, ErrNotFound
		}
